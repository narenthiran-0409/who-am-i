...
3. Reload the portfolio. The API checks disk at most once every five seconds; React revalidates its HTTP cache on each request; other clients may cache responses for up to 60 seconds. It does not push updates into already-open pages.
...
- The request has a four-second timeout. Network, HTTP, invalid JSON or incompatible content errors open the tea-break outage screen. TRY AGAIN revalidates the API; duplicate retries are ignored. VIEW SAVED PORTFOLIO restores cached/bundled content with a reconnect banner. Contact/email errors do not trigger this screen. Browser storage is optional; denied access/quota errors are handled.
...
`src/data/outage.json` holds locally bundled outage copy and the illustration reference. It deliberately stays independent of the unavailable API and its seven-section contract. `ServerTeaBreak` renders real HTML headings and buttons over the original approved illustration cropped with CSS. Floating Zs, steam and cyan status lights animate with CSS; this is not a rigged 3D character. Reduced-motion users receive a static presentation. The heading receives focus when the screen appears, and focus returns to the portfolio on recovery or saved-view selection. The initial portfolio remains visible during the bounded first request, avoiding a false outage flash on healthy connections.

Tea-break update verification: 32 frontend tests pass and the production build succeeds. Browser review covers desktop, 768px tablet and 320px mobile layouts, failed retry and saved-view navigation. API recovery and duplicate/timeout/unmount behavior are covered by React integration tests. Animation rules are gated behind prefers-reduced-motion: no-preference. The approved art is reused unchanged (1.35 MB PNG); effects are CSS overlays. See `screenshots/tea-break-desktop.jpg` for the implemented screen.
